...
# Extract necessary data for later use: 
# CFPS
load("CFPS2010.RData")  # load dataframes: df.children ....
...
betan_psid <- betan_psid %>%
  ...
  dplyr::filter(!is.na(SES_betan_psid))
# Select children
...
moog_child_cfps <- merge(moog_child_cfps, moog_cfps_education, by = "pid_m", all.y = TRUE)
...
yu_cfps_sss <- df.individual %>%
  dplyr::select(pid, qm402)  %>%
  dplyr::filter(qm402 > 0)# pid, subjective social status
...
yu.pr_cfps <- yu_cfps %>%
  ...
  tidyr::drop_na(.) %>%
  FactoMineR::PCA(., scale.unit = TRUE, graph = TRUE)
...
# psid
# only rp and sp have sss and parental education info
# extract sp and rp 
...
key<- -1  # 
jed_cfps_occup$occup_ses <- psych::reverse.code(key, jed_cfps_occup$occup_ses) #reverse the coding
...
# select children
romeo2_child_psid <- merge(romeo2_psid, psid_child, by = "fid", all.y = TRUE) #select children
