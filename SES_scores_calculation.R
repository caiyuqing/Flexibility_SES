...
moog_CFPS <- df.CFPS_child %>%
  ...
  dplyr::mutate(income_cat = base::cut(fincome, #recode income
...
moog_PSID <- df.PSID_child %>%
  ...
  dplyr::mutate(edu_m_recode = cut(edu_m, breaks = c(-0.00001, 11.5, 12.5, 16.5, 98, 100), labels = c("1", "2", "3", "4",  NA))) %>% #recode education: cut into 4 groups: <12, 12, 13-16, 17  (?do not distinguish master and phd)
...
jed_CFPS <- df.CFPS_child %>%
  ...
  dplyr::mutate(edu_m_recode = cut(educ_m, breaks = c(-0.5,4.5,5.5,6.5,10.5,13.5,14.5,16.5), labels = c("1", "2", "3", "4", "5", "6", "7"))) %>%  # recode education
...
mcder_CFPS <- df.CFPS_child %>%
  dplyr::mutate(edu_m_recode = cut(educ_m, breaks = c(-0.5, 3.5, 5.5,6.5,10.5,13.5,14.5,16.5), labels = c("1", "2", "3", "4", "5", "6", "7"))) %>%  # recode education  (mother)
  dplyr::mutate(occup_m_recode = recode(egp_m, "1"= 9, "2"=8, "3"=7,   "4"=6, "5"= 5, "7"= 5, "8"= 4, "9"=3, "10"=2, "11"=1, .default = -8)) %>%    # recode occupation  (mother)
  dplyr::mutate(edu_f_recode = cut(educ_f, breaks = c(-0.5, 3.5, 5.5,6.5,10.5,13.5,14.5,16.5), labels = c("1", "2", "3", "4", "5", "6", "7"))) %>%  # recode education  (father)
...
romeo2_CFPS <- df.CFPS_child %>%
  ...
  dplyr::mutate(edu_f_recode = cut(educ_f, breaks = c(-0.5,6.5,11.5,13.5,14.5,16.4), labels = c("1", "2", "3", "4","5"))) %>%  # calculate composite parents' education score
  dplyr::mutate(edu_f_recode = as.numeric(as.character(edu_f_recode))-1) %>% # recode education 0-4
  dplyr::mutate(edu_m_recode = cut(educ_m, breaks = c(-0.5,6.5,11.5,13.5,14.5,16.4), labels = c("1", "2", "3", "4","5"))) %>%  # calculate composite parents' education score
...
romeo2_PSID <- df.PSID_child %>%
  dplyr::mutate(edu_m_recode = cut(edu_m, breaks = c(-0.00001, 11.5, 12.5,  14.5, 16.5, 98, 100), labels = c("1", "2", "3", "4", "5",  "6"))) %>%  # set NA
  ...
  dplyr::mutate(edu_m_recode = na_if(edu_m_recode, 5),  # set NA
...
leo_CFPS <- df.CFPS_child %>%
  dplyr::mutate(SES_leo_cfps = cut(edu2010_t1_best_m, breaks = c(0, 4.5, 8.5), labels = c("1", "2"))) %>%  # recode education
...
leo_PSID <- df.PSID_child %>%
  dplyr::mutate(SES_leo_psid = cut(edu_m, breaks = c(-0.01, 12.5, 17.5, 100), labels = c("1", "2", "3"))) %>%  # recode education
  dplyr::mutate(SES_leo_psid = as.numeric(as.character(SES_leo_psid))) %>%  # convert it to numeric one
...
ozer_CFPS <- df.CFPS_child %>%
  dplyr::mutate(edu_m_recode = cut(educ_m, breaks = c(-0.5, 3.5, 5.5,6.5,10.5,13.5,14.5,16.5), labels = c("1", "2", "3", "4", "5", "6", "7"))) %>%  # composite education score of parents
  ...
  dplyr::mutate(SES_ozer_cfps = cut(edu_parents, breaks = c(0, median(edu_parents, na.rm = 10), 100), labels = c('1', '2')))%>%  #recode education
...
ozer_PSID <- df.PSID_child %>%
  ...
  dplyr::mutate(SES_ozer_psid = cut(edu_parents, breaks = c(0, median(edu_parents, na.rm = 10), 100), labels = c('1', '2')))%>%  #convert SES score into numeric
