...
#           urban area: use "The total income of poverty-stricken urban citizens (5%) per person" (see https://data.stats.gov.cn/easyquery.htm?cn=C01&zb=A0A0802&sj=2010): (total family income (year) CNY5483.1/per person)
...
#
# Note: in CFPS and PSID, cannot distinguish "Techical/Vocational (3);some college (4)" levels. Only 6 levels
## CFPS ##
betan_CFPS <- df.CFPS_child %>%
  dplyr::mutate(itn = ifelse(urban == 0, base::cut(faminc/familysize, breaks = c(-0.00001, 1274, 1274*2, 1274*3, 1274*4, Inf), labels = c("1", "2", "3", "4", "5")),
                             base::cut(faminc/familysize, breaks = c(-0.00001, 5483.1, 5483.1*2, 5483.1*3, 5483.1*4, Inf), labels = c("1", "2", "3", "4", "5")))) %>%   # set the ITN for every individual
  # set 7 levels for mother's education
  dplyr::mutate(edu_m_recode = base::cut(eduy_m, breaks = c(-0.01, 9.5, 12.5, 13.5, 14.5, 16.5, 22.5), labels = c("1", "2", "3", "4", "5", "6"))) %>% 
...
betan_PSID <- df.PSID_child %>%
  dplyr::mutate(edu_m_recode = cut(edu_m, breaks = c(-0.01, 9.5, 12.5, 13.5, 14.5, 16.5, 17.5, 100), 
                                   labels = c("1", "2", "3", "4", "5", "6", NA)),  # set 7 levels for mtoher's education
                edu_m_recode = as.numeric(as.character(edu_m_recode))) %>% 
  dplyr::mutate(itn1 = 12060 +  (familysize-1)*4180) %>% # set the poverty line for every family: poverty line 12060 for one people and increase 4180 for an extra person
...
moog_CFPS <- df.CFPS_child %>%
  ...
  dplyr::mutate(income_cat = base::cut(faminc, #recode income
                               breaks= c(-0.01, 15000*1.013*3.329, 45000*1.013*3.329, 75000*1.013*3.329, 100000*1.013*3.329, 300000), # CPI US 2010/2008 = 218.056/215.303 = 1.013; PPP China CNY/US dollar (2010) = 3.329
                               labels = c("1", "2", "3", "4", "5"))) %>% 
...
moog_PSID <- df.PSID_child %>%
  dplyr::mutate(income_cat=cut(fincome, breaks= c(-0.01, 15000*1.139, 45000*1.139, 75000*1.139, 100000*1.139, 300000), labels = c("1", "2","3", "4","5"))) %>%#recode income:CPI US 2017/2008 = 245.120/215.303 = 1.139;
  dplyr::mutate(edu_m_recode = cut(edu_m, breaks = c(-0.00001, 8.5, 12.5, 14.5, 16.5, 17.5, 100), labels = c("1", "2", "3", "4",  "5", NA))) %>% #recode education: cut into 5 groups
...
jed_CFPS <- df.CFPS_child %>%
  dplyr::mutate(occup_ses = recode(egp_m, "1"= 7, "2"=6, "3"=5,  "7"= 5, "4"=4, "5"= 4, "6"= 4, "8"= 3, "9"=2, "10"=1, "11"=1, "-8"=-8,"80000"= 8, .default = -8)) %>%  # recode occupation
  dplyr::mutate(edu_m_recode = cut(eduy_m, breaks = c(-0.01,3.5,6.5,9.5,12.5,14.5,16.5, 22.5), labels = c("1", "2", "3", "4", "5", "6", "7"))) %>%  # recode education into 7 levels
...
mcder_CFPS <- df.CFPS_child %>%
  dplyr::mutate(edu_m_recode = cut(eduy_m, breaks = c(-0.5, 7.5, 9.5,11.5,12.5,14.5,16.5,22.5), labels = c("1", "2", "3", "4", "5", "6", "7"))) %>%  # recode education  (mother)
  dplyr::mutate(occup_m_recode = recode(egp_m, "1"= 9, "2"=8, "3"=7,   "4"=6, "5"= 5, "7"= 5, "8"= 4, "9"=3, "10"=2, "11"=1, .default = -8)) %>%    # recode occupation  (mother)
  dplyr::mutate(edu_f_recode = cut(eduy_f, breaks = c(-0.5, 7.5, 9.5,11.5,12.5,14.5,16.5,22.5), labels = c("1", "2", "3", "4", "5", "6", "7"))) %>%  # recode education  (father)
  dplyr::mutate(occup_f_recode = recode(egp_f, "1"= 9, "2"=8, "3"=7,   "4"=6, "5"= 5, "7"= 5, "8"= 4, "9"=3, "10"=2, "11"=1, .default = -8)) %>%    # recode occupation (father)
  dplyr::na_if(., -8) %>%  # set NA
...
romeo1_CFPS <- df.CFPS_child %>%
  dplyr::mutate(edu_m_recode = cut(eduy_m, breaks = c(-0.5, 6.5, 9.5,11.5,12.5,14.5,16.5,22.5), labels = c("1", "2", "3", "4", "5", "6", "7"))) %>%  # recode education
  dplyr::mutate(occu_m_recode = recode(egp_m, "1"= 9, "2"=8, "3"=7,   "4"=6, "5"= 5, "7"= 5, "8"= 4, "9"=3, "10"=2, "11"=1,  .default = -8)) %>%  # recode occupation
  dplyr::na_if(., -8) %>%  # set NA
...
# education: recode into 5 levels (0-4) and then calculate z-score of the parents' education (replace parents' education with each other if one of them is NA)

## CFPS ##
romeo2_CFPS <- df.CFPS_child %>%
  # recode income 
  dplyr::mutate(income_zscore = (faminc - mean(faminc, na.rm = TRUE))/sd(faminc, na.rm = TRUE)) %>%
  dplyr::mutate(edu_f_recode = cut(feduc, breaks = c(-0.5,3.5,4.5,5.5,6.5,8.5), labels = c("1", "2", "3", "4","5"))) %>%  # calculate composite parents' education score
  dplyr::mutate(edu_f_recode = as.numeric(as.character(edu_f_recode))-1) %>% # recode education 0-4
  dplyr::mutate(edu_m_recode = cut(meduc, breaks = c(-0.5,3.5,4.5,5.5,6.5,8.5), labels = c("1", "2", "3", "4","5"))) %>%  # calculate composite parents' education score
...
romeo2_PSID <- df.PSID_child %>%
  dplyr::mutate(edu_m_recode = cut(edu_m, breaks = c(-0.00001, 8.5, 12.5,  14.5, 16.5, 17.5, 100), labels = c("1", "2", "3", "4", "5",  NA))) %>%  # set NA
  dplyr::mutate(edu_f_recode = cut(edu_f, breaks = c(-0.00001, 8.5, 12.5,  14.5, 16.5, 17.5, 100), labels = c("1", "2", "3", "4", "5",  NA))) %>%
...
qiu_CFPS <- df.CFPS_child%>%
  dplyr::rename(SES_qiu_cfps = faminc)
...
knick_CFPS <- df.CFPS_child %>%
  dplyr::mutate(FPL = ifelse(urban ==0, (faminc/familysize)/1274, (faminc/familysize)/5483.1)) %>%   # calculate FPL
  dplyr::mutate(SES_knick_cfps_inc = cut(FPL, breaks = c(0, 2, 4, Inf), labels = c("1", "2", "3")))%>%   #200%, 400% of poverty line as cut point
...
lawson_CFPS <- df.CFPS_child %>%
  dplyr::mutate(edu_f_recode = cut(feduc, breaks = c(-0.01, 3.5, 4.5,5.5,6.5,7.5, 8.5), labels = c("1", "2", "3", "4", "5", "6")),
                edu_m_recode = cut(meduc, breaks = c(-0.01, 3.5, 4.5,5.5,6.5,7.5, 8.5), labels = c("1", "2", "3", "4", "5", "6"))) %>%
...
lawson_PSID <- df.PSID_child %>%
  dplyr::mutate(edu_f_recode = cut(edu_f, breaks = c(-0.01, 8.5, 12.5,14.5, 16.5, 17.5, 100), labels = c("1", "2", "3", "4", "5", NA)),
                edu_m_recode = cut(edu_m, breaks = c(-0.01, 8.5, 12.5,14.5, 16.5, 17.5, 100), labels = c("1", "2", "3", "4", "5", NA))) %>% # 6 = NA; did not distinguish "some graduate" and "graduate" in PSID (only 5 levels)
  dplyr::mutate(edu_f_recode = as.numeric(as.character(edu_f_recode)),
                edu_m_recode = as.numeric(as.character(edu_m_recode))) %>%
...
##      education: (in 7 levels, see education & occupation.xlsx) and transformed into years (?not certain whether to composite parents education (currently use composite))
## Note: two separate SESs
##       Although education was in years in orginal paper, it was transformed from categorical variable of education, so we also use categorical education index (fedu, medu) in CFPS 

## CFPS ##
hanson2_CFPS <- df.CFPS_child %>%
  dplyr::mutate(edu_f_recode = cut(feduc, breaks = c(-0.01, 2.5, 3.5, 4.5, 5.5, 6.5,7.5, 8.5), labels = c("5", "11", "12", "14", "16", "17", "19")),
                edu_m_recode = cut(meduc, breaks = c(-0.01, 2.5, 3.5, 4.5, 5.5, 6.5,7.5, 8.5), labels = c("5", "11", "12", "14", "16", "17", "19"))) %>%
  dplyr::mutate(edu_f_recode = as.numeric(as.character(edu_f_recode)),
                edu_m_recode = as.numeric(as.character(edu_m_recode))) %>%
  dplyr::mutate(SES_hanson2_cfps_edu = (edu_m_recode + edu_f_recode)/2) %>%
  dplyr::mutate(fincome_recode = cut (faminc, breaks = c(-0.01, 5000*0.96*3.329, 10000*0.96*3.329, 15000*0.96*3.329, 35000*0.96*3.329, 50000*0.96*3.329, 75000*0.96*3.329, 100000*0.96*3.329, 5000000*0.96*3.329),
                                      labels = c("4500","7500", "12500", "25000", "42500", "62500", "87500", "112500"))) %>% # CPI US dollar 2010/2011 =218.056/224.939 =0.96 , PPP CNY/US dollar in 2010 = 3.329
  dplyr::mutate(fincome_recode = as.numeric(as.character(fincome_recode))) %>%
  dplyr::mutate(SES_hanson2_cfps_inc = log10(fincome_recode*0.96*3.329)) # convert the midpoint also to the equivalence of the CNY and log transform it
...
hanson2_PSID <- df.PSID_child %>%
  dplyr::mutate(edu_f_recode = cut(edu_f, breaks = c(-0.01,6.5, 11.5, 12.5,14.5,16.5, 17.5, 100), labels = c("5", "11", "12", "14", "16", "17", NA)),
                edu_m_recode = cut(edu_m, breaks = c(-0.01,6.5, 11.5, 12.5,14.5,16.5, 17.5, 100), labels = c("5", "11", "12", "14", "16", "17", NA))) %>% #  did not distinguish "some graduate" and "graduate" in PSID (only 6 levels)
  dplyr::mutate(edu_f_recode = as.numeric(as.character(edu_f_recode)),
                edu_m_recode = as.numeric(as.character(edu_m_recode))) %>%
  dplyr::mutate(SES_hanson2_psid_edu = (edu_m_recode + edu_f_recode)/2) %>%
  dplyr::mutate(fincome_recode = cut (fincome, breaks = c(-0.01, 5000*1.090, 10000*1.090, 15000*1.090, 35000*1.090, 50000*1.090, 75000*1.090, 100000*1.090, 5000000*1.090), labels = c("4500", "12500", "20000", "30000", "42500", "62500", "87500", "112500"))) %>% # CPI 2017/2011 US dollar = 245.120/224.939 = 1.090
  dplyr::mutate(fincome_recode = as.numeric(as.character(fincome_recode))) %>%
...
noble_CFPS <- df.CFPS_child %>%
  dplyr::mutate(fincome_recode = cut(faminc, breaks = c(-0.01, 5000*0.92*3.329, 9999*0.92*3.329, 19999*0.92*3.329, 
                                                          29999*0.92*3.329, 39999*0.92*3.329, 49999*0.92*3.329, 99999*0.92*3.329,
                                                          149999*0.92*3.329, 199999*0.92*3.329, 249999*0.92*3.329, 299999*0.92*3.329, 5000000*0.92*3.329), 
                                     labels = c("4500", "7500", "15000", "25000", "35000", "45000", 
                                                "75000", "125000","175000", "225000", "275000", "325000"))) %>% #Incomes were  divided into 12 levels, CPI US dollar 2010/2015 = 218.056/237.017 = 0.92; PPP: CNY/US dollar in 2010 = 3.329
  dplyr::mutate(fincome_recode = as.numeric(as.character(fincome_recode))) %>%
...
noble_PSID <- df.PSID_child %>%
  ...
  dplyr::mutate(edu_f_recode = cut(edu_f, breaks = c(-0.01, 6.5, 9.5, 11.5, 12.5,14.5, 16.5, 17.5, 100), labels = c("6","8", "10.5", "12", "14", "16", "18", NA)),
                edu_m_recode = cut(edu_m, breaks = c(-0.01, 6.5, 9.5, 11.5, 12.5,14.5, 16.5, 17.5, 100), labels = c("6","8", "10.5", "12", "14", "16", "18", NA))) %>%
  dplyr::mutate(edu_f_recode = as.numeric(as.character(edu_f_recode)),
                edu_m_recode = as.numeric(as.character(edu_m_recode))) %>%
...
ell_CFPS <- df.CFPS_child %>%
  dplyr::mutate(fincome_recode = cut(faminc, breaks = c(-0.01, 10000*3.329*0.87, 25000*3.329*0.87, 50000*3.329*0.87, 75000*3.329*0.87, 100000*3.329*0.87, 5000000*3.329*0.87), 
  ...
  dplyr::mutate(edu_f_recode = cut(eduy_f, breaks = c(-0.01, 9.5, 12.5,13.5, 14.5, 16.5,19.5, 22.5), labels = c("9","12", "13", "14", "16", "18", "21")),
                edu_m_recode = cut(eduy_m, breaks = c(-0.01, 9.5, 12.5,13.5, 14.5, 16.5,19.5, 22.5), labels = c("9","12", "13", "14", "16", "18", "21"))) %>%
  dplyr::mutate(edu_f_recode = as.numeric(as.character(edu_f_recode)),
                edu_m_recode = as.numeric(as.character(edu_m_recode))) %>%
...
ell_PSID <- df.PSID_child %>%
  dplyr::mutate(fincome_recode = cut(fincome, breaks = c(-0.01, 10000*0.97, 25000*0.97, 50000*0.97, 75000*0.97, 100000*0.97, 5000000*0.97), 
  ...
  dplyr::mutate(edu_f_recode = cut(edu_f, breaks = c(-0.01, 8.5,12.5, 13.5, 14.5,16.5, 17.5, 100), labels = c("8","12", "13", "14", "16", "19.5", NA)),
                edu_m_recode = cut(edu_m, breaks = c(-0.01, 8.5,12.5, 13.5, 14.5,16.5, 17.5, 100), labels = c("8","12", "13", "14", "16", "19.5", NA))) %>% #similar to the previous ones, no distinguish between master and doctorate (only 6 levels, last level set to 19.5)
  dplyr::mutate(edu_f_recode = as.numeric(as.character(edu_f_recode)),
                edu_m_recode = as.numeric(as.character(edu_m_recode))) %>%
...
luby_CFPS <- df.CFPS_child %>%
  ...
  dplyr::mutate(edu_f_recode = cut(feduc, breaks = c(-0.01, 3.5,4.5,5.5, 6.5,8.5), labels = c("1","2", "3", "4", "5")),
                edu_m_recode = cut(meduc, breaks = c(-0.01, 3.5,4.5,5.5, 6.5,8.5), labels = c("1","2", "3", "4", "5"))) %>% #similar to the previous ones, no distinguish between master and doctorate (only 6 levels, last level set to 19.5)
  dplyr::mutate(edu_f_recode = as.numeric(as.character(edu_f_recode)),
                edu_m_recode = as.numeric(as.character(edu_m_recode))) %>%
...
luby_PSID <- df.PSID_child %>%
  ...
  dplyr::mutate(edu_f_recode = cut(edu_f, breaks = c(-0.01, 8.5,12.5,14.5, 16.5,17.5, 100), labels = c("1","2", "3", "4", "5", NA)),
                edu_m_recode = cut(edu_m, breaks = c(-0.01, 8.5,12.5,14.5, 16.5,17.5, 100), labels = c("1","2", "3", "4", "5", NA))) %>% #similar to the previous ones, no distinguish between master and doctorate (only 6 levels, last level set to 19.5)
  dplyr::mutate(edu_f_recode = as.numeric(as.character(edu_f_recode)),
                edu_m_recode = as.numeric(as.character(edu_m_recode))) %>%
...
takeu_CFPS <- df.CFPS_child %>%
  dplyr::mutate(income_cat = cut(faminc, breaks = c(-0.01, 2000000*0.94*0.03, 4000000*0.94*0.03, 6000000*0.94*0.03, 8000000*0.94*0.03, 10000000*0.94*0.03, 12000000*0.94*0.03, 5000000), labels = c("1", "2", "3", "4", "5", "6", "7"))) %>% #CPI: Japan yen 2010/2019 = 100/105.482 = 0.94; PPP CNY/Japan yen 2010 = 3.329/111.667 = 0.03
  dplyr::mutate(SES_takeu_cfps_inc = as.numeric(as.character(income_cat))) %>%
  dplyr::mutate(edu_f_recode = cut(eduy_f, breaks = c(-0.01, 6.5,9.5,11.5,12.5, 14.5, 16.5, 19.5,22.5), labels = c("6","9", "11", "12", "14", "16", "18", "21")),
                edu_m_recode = cut(eduy_m, breaks = c(-0.01, 6.5,9.5,11.5,12.5, 14.5, 16.5, 19.5,22.5), labels = c("6","9", "11", "12", "14", "16", "18", "21"))) %>% 
  dplyr::mutate(edu_f_recode = as.numeric(as.character(edu_f_recode)),
                edu_m_recode = as.numeric(as.character(edu_m_recode))) %>%
...
takeu_PSID <- df.PSID_child %>%
  ...
  dplyr::mutate(edu_f_recode = cut(edu_f, breaks = c(-0.01, 6.5,8.5,11.5,12.5, 14.5, 16.5, 17.5,99), labels = c("6","9", "11", "12", "14", "16", "19.5", NA)),
                edu_m_recode = cut(edu_m, breaks = c(-0.01, 6.5,8.5,11.5,12.5, 14.5, 16.5, 17.5,99), labels = c("6","9", "11", "12", "14", "16", "19.5", NA))) %>% # not distinguish "master" and "doctorate", only 7 levels (highest level: 19.5)
  dplyr::mutate(edu_f_recode = as.numeric(as.character(edu_f_recode)),
                edu_m_recode = as.numeric(as.character(edu_m_recode))) %>%
